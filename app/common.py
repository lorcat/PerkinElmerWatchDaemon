...
class Logger(object):
    ...
    def __init__(self, def_file = None, debug_level = None, nofile = False):
        if debug_level is not None:
            self.DEFAULTLEVEL = debug_level

        logging.basicConfig(level=self.DEFAULTLEVEL,
                            format='%(asctime)s %(levelname)-8s %(process)-8d %(thread)-10d %(threadName)-16s %(name)-12s: %(message)s',
                            datefmt='%m-%d %H:%M',
                            filemode='w')
        ...
        if def_file is None:
            ...
            self._logger = logging.getLogger("{}/{}".format(self.__class__.__name__, os.path.basename(str(def_file))))
        ...
        try:
            # do not create file handles unless necessary
            if not nofile:
    ...
    def addFileHandler(self, filename):
        ...
        # add formatter
        fmtr = logging.Formatter('%(asctime)s %(levelname)-8s %(process)-8d %(thread)-10d %(threadName)-16s %(name)-12s: %(message)s')
...
class MutexLock(Tester):
    DEFAULT_FILE = "lock"
    DEFAULT_DIR = config.DIR_TEMPFILES
    DEF_LOCK_DIR = config.DIR_LOCKFILES
    ...
    def __init__(self, def_file=None, debug_level=None):
        ...
        temp = [self.DEF_LOCK_DIR, def_file]
        ...
        if not os.path.exists(self.DEFAULT_DIR) or not os.path.isdir(self.DEFAULT_DIR):
            ...
            self.lock_dir = os.path.join(os.path.dirname(__file__), self.DEF_LOCK_DIR)
        else:
    ...
    def lock(self):
        ...
        """
        res = True
        self.debug("Locking the execution")
    ...
    def unlock(self):
        ...
        if os.path.exists(self.lock_name) and os.path.isfile(self.lock_name):
            ...
            self.warning("A lock file entry is not a file ({})".format(self.lock_name))
        return res
...
# TODO: Disable log file for threads - get TOO many open files
# TODO: Each thread gets a name corresponding to the plugin - test that threads are started as they should